...
func TestAppendPlatformCookiesInstagramFile(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	cookieFile := filepath.Join(dir, "instagram_cookies.txt")
	if err := os.WriteFile(cookieFile, []byte("test"), 0o600); err != nil {
		t.Fatal(err)
	}
	args := appendPlatformCookies(nil, Options{
...
func TestAppendPlatformCookiesInstagramBrowserFallback(t *testing.T) {
	t.Parallel()
	args := appendPlatformCookies(nil, Options{
		Platform:                    "instagram",
