...
// Main is the shared entrypoint for single-bot service binaries: it loads
// config, sets up logging and runs the bot until termination.
func Main(bc BotConfig) {
	cfg, err := config.Load()
	if err != nil {
	...
	if err := Run(context.Background(), cfg, &bc); err != nil {
		slog.Error("application stopped", "err", err, "bot", bc.Slug)
		os.Exit(1)
	}
}

// Run wires bot + worker + metrics for one bot according to SAVEINATOR_MODE.
func Run(ctx context.Context, cfg *config.Settings, bc *BotConfig) error {
	...
	if mode == "bot" || mode == "all" {
		go func() { errCh <- runBot(ctx, bot, store, redisClient, cfg, bc) }()
	}
	if mode == "worker" || mode == "all" {
		go func() { errCh <- runWorker(ctx, bot, store, redisClient, cfg, bc) }()
	}
...
func runBot(ctx context.Context, bot *telego.Bot, store *db.Store, redisClient *redisx.Client, cfg *config.Settings, bc *BotConfig) error {
	...
	New(bc, cfg, store, redisClient, q).Register(bh, bot)
	...
	slog.Info("starting webhook mode", "addr", addr, "bot", bc.Slug)
...
func runWorker(ctx context.Context, bot *telego.Bot, store *db.Store, redisClient *redisx.Client, cfg *config.Settings, bc *BotConfig) error {
	...
	deps := botworker.NewDeps(cfg, snd, store, redisClient)
	for _, p := range bc.Platforms {
		p.RegisterWorker(mux, deps, bc)
	}
