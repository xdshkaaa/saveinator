...
import (
	...
	pinteresthandler "saveinator/services/pinterest_kz/handler"
	pinterestworker "saveinator/services/pinterest_kz/worker"
)
...
func runBot(ctx context.Context, bot *telego.Bot, store *db.Store, redisClient *redisx.Client, cfg *config.Settings) error {
	...
	if cfg.UsePolling {
		slog.Info("starting polling mode (pinterest_kz)")
	...
	slog.Info("starting webhook mode (pinterest_kz)", "addr", addr)
...
func runWorker(ctx context.Context, bot *telego.Bot, store *db.Store, redisClient *redisx.Client, cfg *config.Settings) error {
	...
	if err := queue.RecoverOrphanedActiveTasks(cfg.RedisURL, queue.PinterestKZQueueName); err != nil {
	...
	slog.Info("starting asynq worker (pinterest_kz)")
	return srv.Run(mux)
}
